...
func FncPsglstErrlogGetall(c *gin.Context) {
	...
	tablex := fncApndix.Client.Database(fncApndix.Dbases).Collection("psglst_logerr")
...
func FncPsglstActlogGetall(c *gin.Context) {
	...
	// Select database and collection
	tablex := fncApndix.Client.Database(fncApndix.Dbases).Collection("psglst_logact")
	contxt, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
