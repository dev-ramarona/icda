		...
		for _, freetx := range psglst.FreeTextInfoList {
			...
			case "ET":
				...
				if len(partsl) >= 6 {
					...
					strDatevc := fncApndix.FncApndixAddfmtYearnw(partsl[2])
			...
			case "BT":
				...
				if rslmkg := regmkg.FindAllString(freetx.TextLine, -1); len(rslmkg) > 0 {
					...
					fncApndix.FncApndixUpdateSlcstr(&objPsglst.Nmbrbt, partsl[2])
				}

			// Comment
			case "CM":
				fncApndix.FncApndixUpdateSlcstr(&objPsglst.Coment, freetx.TextLine)
				...
				for _, prdata := range partsl {
					...
					case regexp.MustCompile(`^\d{1,2}[A-Z]{3}(\d{0,2})?$`).MatchString(prdata):
						if len(prdata) >= 6 {
							...
							strdte = fncApndix.FncApndixAddfmtYearnw(prdata)
						}
			...
			case "IB":
				...
				if len(partsl) >= 5 {
					...
					strIbdate := fncApndix.FncApndixAddfmtYearnw(partsl[4])
			...
			case "IFET":
				if strings.Contains(freetx.TextLine, "IFET-") {
					...
					strIfdate := fncApndix.FncApndixAddfmtYearnw(partsl[2])
