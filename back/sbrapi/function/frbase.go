...
func FncSbrapiFrbaseTrtmnt(rawxml []mdlSbrapi.MdlSbrapiFrbaseRspfbs,
	...
	for _, frbase := range rawxml {
		...
		if val, ist := sycFrbase.Load(tmpPrmkey); ist {
			if get, mtc := val.(mdlApndix.MdlApndixFrbaseDtbase); mtc {
				nowDatend, nowHstory = fncApndix.FncApndixFormatHstory(get.Frbsbr,
		...
		// Final result
		nowFrbase := mdlApndix.MdlApndixFrbaseDtbase{
			Prmkey: tmpPrmkey,
			Scdkey: tmpScdkey,
