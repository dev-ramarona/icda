...
// Get object slice
func FncApndixMilegeGetall(c *gin.Context) {
	...
	var mtchdt = bson.A{}
	var sortdt = bson.D{{Key: "$sort", Value: bson.D{{Key: "milege", Value: 1}}}}
	...
	go func() {
		...
		// Store to slice from raw bson
		var slctmp = []mdlApndix.MdlApndixMilegeFrntnd{}
		for rawDtaset.Next(contxt) {
			slcDtaset := mdlApndix.MdlApndixMilegeDtbase{}
			rawDtaset.Decode(&slcDtaset)
			slctmp = append(slctmp, mdlApndix.MdlApndixMilegeFrntnd{
				Prmkey: slcDtaset.Routfl,
				Routfl: slcDtaset.Routfl,
				Milege: slcDtaset.Milege,
...
func FncApndixMilegeUpdate(c *gin.Context) {

	// Bind JSON Body input to variable
	var inputx mdlApndix.MdlApndixMilegeDtbase
	if err := c.BindJSON(&inputx); err != nil {
	...
	// Get from input
	if inputx.Milege == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid input Milege"})
		return
	}
	if inputx.Routfl == "" {
...
// Download
func FncApndixMilegeDownld(c *gin.Context) {
	...
	c.Header("Content-Disposition", "attachment; filename=apndix_Milege_"+fnlFilenm+".csv")
	...
	countr := 0
	for rawDtaset.Next(contxt) {
		var slcDtaset mdlApndix.MdlApndixMilegeDtbase
		rawDtaset.Decode(&slcDtaset)

		// Write to CSV
		writer.Write([]string{
			slcDtaset.Routfl,
			fmt.Sprintf("%v", slcDtaset.Milege),
			slcDtaset.Updtby,
		})
