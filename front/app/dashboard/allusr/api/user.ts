...
export async function ApiAllusrUsrlstGetall(prmAllusr: MdlAllusrSearchParams) {
  const tag = ["allusr", prmAllusr.pagenw, prmAllusr.update]
    .filter(Boolean)
    .join(":");
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_URL_SERVER}/allusr/getall`,
      {
        method: "POST",
...
export async function ApiAllusrHandleDelete(usrnme: string) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_URL_SERVER}/allusr/delete/${usrnme}`,
      {
        method: "GET",
      },
    );
